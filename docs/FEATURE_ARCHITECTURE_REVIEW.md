...
| F12 | 按创作来源分类 | 已调整 | 展馆按单图/系列筛选, 系列根据输入内容策划, 取消四类场景模板 |
...
| F25 | 切图批量下载 | 已实现 | 将当前完成批次的全部切片打包为一个 ZIP, 保留单张下载与送创作; 校验数量, 命名, CRC, 图片内容与实际格式, 修改参数或换图不得混用旧批次 |
| F26 | 已选模板预览 | 已实现 | 用示例图/名称紧凑卡片替换旧标签, 提供更换与移除, 不在创作卡片展示作者/分类/常驻说明, 在风格库保留署名; 新选择保存预览快照, 移除关联保留文案, 不自动将示例图作为生成参考 |
| F27 | 单图多参考输入 | 已实现 | 最多 4 张, 单张 12 MiB, 多选/追加/单张替换/删除, 草稿与配方完整恢复; 局部重绘选择一张原图, 其余图片留存草稿, 按输出张数计费 |
...
| B13 | 单图画卷堆积全部作品, 左侧滚轮先带动右侧 | 限制为最近 4 个记录, 当前批次优先, 其他未完成任务保留, 不足补最近作品; 任务/结果原位替换. 取消左侧吸顶并使用整页滚动, 从独立入口访问完整展馆 |
...
| B17 | 上传时误显示失败, 迟到提交响应恢复已领取占位 | 区分提交中与待确认, 沿用原请求编号恢复; 忽略过时队列快照, 受理响应只关联现存 outbox, 不重新写入已完成记录 |
| B18 | 下载原图阻塞队列, 系列待领取分镜被视为待生成 | 独立轮询和结果领取, 同页面最多领取 2 项; 分别展示下载/保存/确认及领取错误, 下载失败只重试领取, 原图未保存的分镜保持处理中 |
| B19 | 个人排队位次易被误解为全站名次, 秒数随轮询跳动, 用户队列清空后轮换重置 | 将全站生成中/等待总数集中在我的任务抽屉, 单图与系列仅显示个人状态, 顺序和耗时; 使用服务端时间基准和浏览器每秒计时, 断连标注上次状态; 用户离开等待列表后继续原轮换位置 |
| B20 | 画卷与我的任务的固定比例进度条容易让人误认为卡住 | 移除比例条, 使用静态等待图标, 处理活动图标和真实耗时; 领取错误停止动画并保留重试入口, 尊重减少动态效果设置 |
| B21 | 离开系列页或本地保存失败时拆解结果丢失 | 草稿事务成功后再结束文字领取, 保留原请求供免费重领, 阻止迟到结果覆盖其他标签页 |
| B22 | 已完成系列重新拆解仍关联旧图 | 成功拆解建立新系列与分镜身份, 清除旧计划任务及单镜覆盖, 保留展馆原作品 |
| B23 | 单镜参数显示默认值, 排队编辑丢失原参考素材 | 读取原任务或作品配方, PATCH 只修改提示词及输出参数, 保留身份, 参考链与点值快照; 锁定已提交批次的全局设置 |
| B24 | 更新回包晚于图片保存时旧草稿被重新写回 | 显式保存的排队编辑不再次触发自动写入, 关联完成后保持草稿清空 |
| B25 | 首镜重试后后续任务仍引用失败首镜, 新版本失败误报整套完成 | 重试引用同系列同分镜的恢复任务; 单独呈现失败与领取异常, 不混入普通批量生成, 未知结果重新生成前确认 |

## 4. 多余功能与清理结果

| 对象 | 结果 |
|---|---|
| 单图结果悬浮条上的额外切图按钮 | 移至作品详情, 保留纯前端切图能力 |
...
- [x] 将画卷限制为最多 4 个记录, 当前批次优先, 其他未完成任务保留, 不足补最近作品. 验证排队/生成/保存/失败/成功原位更新, 连续提交不覆盖上一张未完成任务, 迟到结果与失败重试去重, 多图输出不挤走本批其他任务, 全部作品和旧任务仍可从展馆与队列访问.
...
- [x] 移除删除, 重置和待核实结算的原因输入, 保留一次确认, 服务端自动记录操作说明. 验证默认原因, 旧码撤销, 原余额与历史流水保留, 重放不重复重置或删除; 保留运行中/待核实任务的删除限制.
...
### 8.10 切图打包, 模板预览与多图参考验收, 2026-09-13

- [x] 验证切片 ZIP 数量, 唯一行列文件名, CRC 和图片字节, 覆盖 PNG/JPEG/WebP 与单张下载. 核对不可整除尺寸的像素边界, 修改行列/格式后下载仍对应已完成批次, 换图清理旧结果.
- [x] 验证模板示例图, 名称与作者展示, 文案编辑及刷新恢复, 移除关联保留提示词, 示例图不作为生成参考, 使用本地图标且手机无横向溢出.
- [x] 验证四图上限, 多选/追加/替换/删除, 非法文件批次不部分写入, 编号与刷新顺序一致; 多图配方/outbox 原图完整, 浏览器刷新后继续领取, 输入四图且输出一张只扣一次图片点数.
- [x] 验证局部重绘先选原图, 选择期间禁止按钮与快捷键生成, 蒙版请求仅包含选中原图, 其他参考图留存并可切回; 服务端拒绝多原图蒙版输入, Images 与 Responses 保留多参考顺序.
- [x] 通过 104 项 Node 测试及 TypeScript/Vite 构建, 本机 Chrome 与同版本 Linux Chromium 各 89 组浏览器检查, Node 22.23.2 独立 Docker 13 项验收. 使用隔离数据与虚拟上游, 不将协议验收表述为真实多图融合效果保证.
- [x] 在无活动图片/文字与未领取图片时备份并更新本机 Docker. 核对原有 79 条任务, 59 款风格, 7 条访问码记录(含历史删除项)及点数账本摘要保持一致, 配置与示例图哈希不变; 本机 `/ready` 返回 HTTP 200, 实际提供的 JS/CSS 与最终构建一致. 不修改真实凭据, 不调用付费模型.

按 `/tmp/sprout-studio-full-regression/`, `/tmp/sprout-studio-linux-regression/`, `/tmp/sprout-studio-final-verification/`, `/tmp/sprout-studio-docker-verification/` 和 `/tmp/sprout-studio-deployment/report.json` 中的报告核对验收范围. 保留升级前备份 `$HOME/.local/share/sprout-canvas/backups/studio-tools-j2htpd92/`. 将本机应用验收与公网入口分别检查: 当前公网 HTTP 530, 隧道无就绪连接; 同网络探测确认 IPv6 的 7844 端口连接超时, IPv4 在 TLS 握手时被重置, 需处理网络出口后重新验证公网访问.

### 8.11 模板卡片与重绘入口精简验收, 2026-09-14

- [x] 将模板卡片收紧为示例图, 名称和更换/移除操作, 移除作者, 分类与底部常驻说明, 在风格库保留署名. 验证刷新恢复和移除关联保留文案, 更新 README 单图预览.
- [x] 在参考模式隐藏重复的涂抹按钮, 通过顶部局部重绘进入; 编辑模式按蒙版状态显示绘制蒙版或编辑蒙版. 验证单图上传/粘贴后的入口, 保存/清空蒙版和作品编辑入口.
- [x] 沿用单份创作草稿和多图选一张原图的流程, 不引入双模式独立草稿. 验证选择期间阻止生成, 请求只提交所选原图与蒙版, 其他参考完整保留, 切回参考模式恢复原顺序.
- [x] 通过 104 项 Node 测试与 TypeScript/Vite 构建, 本机 Chrome 和同版本 Linux Chromium 各 46 组直接相关浏览器回归, Node 22.23.2 独立 Docker 13 项验收. 全部模型调用使用隔离数据与虚拟上游.
- [x] 在无活动图片/文字与未领取图片时完成 SQLite 在线备份并更新本机应用容器. 核对 79 条任务, 59 款风格, 7 条访问码记录(含历史删除项)及账本摘要保持一致; 配置和示例图哈希不变, 本机健康检查与构建文件一致性通过, 隧道容器和配置保持不变.

按 `/tmp/sprout-studio-simplify-tests.log`, `/tmp/sprout-studio-simplify-browser/`, `/tmp/sprout-studio-simplify-linux/`, `/tmp/sprout-studio-simplify-docker/` 和 `/tmp/sprout-studio-simplify-deployment/report.json` 核对本次验收. 保留备份 `$HOME/.local/share/sprout-canvas/backups/studio-simplify-20260914-P3xVBz/`, 按用户授权再提交或推送.

### 8.12 提交与结果领取的慢网验收, 2026-09-16

- [x] 区分提交中和提交待确认, 保持单图占位及系列分镜状态稳定, 不将上传等待误报为生成失败.
- [x] 分离元信息轮询与原图领取, 同页面最多领取 2 项且单任务去重. 分别展示下载/保存/确认, 领取失败只重试原结果, 不重新调用模型或扣点.
- [x] 验证提交响应丢失后, 即使轮询已关联服务端任务, 刷新续交仍使用原批次编号. 验证迟到队列快照不覆盖新受理任务, 迟到提交响应不重建已经清理的 outbox.
- [x] 验证原图慢下载期间继续查询队列并提交其他任务, 本地多图事务失败不提前 ack, 确认响应丢失不重复保存, 刷新与换码保留身份隔离及原结果领取.
- [x] 通过 105 项 Node 测试和 TypeScript/Vite 构建, 9 套浏览器脚本共 101 组检查, 同版本 Linux Chromium 的 28 组相关回归, 独立 Docker 的 13 项验收. 使用隔离数据和虚拟上游, 不调用真实付费模型.
- [x] 在无活动图片/文字和待领取结果时更新本机应用容器. 核对 83 条任务元信息及 138 条点数流水保持一致, 核对账本摘要不变且构建文件与验收版本一致, 确认本机首页/健康/就绪接口均返回 200. 保留现有数据库, 图片中转和隧道配置.
- [x] 单独确认公网入口返回 HTTP 530 / Cloudflare 1033, 隧道日志显示 IPv6 边缘连接超时; 将公网恢复继续列为网络验收事项.

按 `/tmp/sprout-queue-fix-verified-test.log`, `/tmp/sprout-queue-fix-verified-browser/`, `/tmp/sprout-queue-fix-verified-linux/`, `/tmp/sprout-queue-fix-verified-docker/` 和 `/tmp/sprout-queue-fix-deployment/report.json` 复核结果. 按用户授权再提交或推送.

### 8.13 系列策划流程与界面验收, 2026-09-16

- [x] 验证拆解结果先保存后结束领取, 切页与保存失败可沿原请求重领, 其他标签页更新草稿后不被迟到结果覆盖.
- [x] 验证重新拆解创建独立计划, 原作品仍在展馆; 单镜参数读取原配方, 排队修改保留参考素材, 请求编号与点值快照.
- [x] 验证保存期间禁用交叉操作, 更新回包晚于图片保存时不恢复已清理的草稿, 首镜失败后后续重试引用恢复后的首镜.
- [x] 验证失败重绘不误报整套完成, 可查看旧版本并修改后重试; 领取失败停止动画且只重领原图; 未知结果取消重新生成后不创建新任务.
- [x] 验证桌面四列, 手机深浅主题和 390x320 矮屏弹窗, 保持真实画幅可见, 错误与重试操作不被固定图片比例裁切.
- [x] 通过 110 项 Node 测试和 TypeScript/Vite 构建, 本机 Chrome 的 78 组相关回归, 同版本 Linux Chromium 的 49 组回归, 独立 Docker 的 13 项验收. 将新增的 12 组系列检查加入默认浏览器验收命令; 所有生成调用使用虚拟上游.
- [x] 在无活动任务及待领取图片时备份数据库并更新本机容器. 核对 84 条任务, 59 条点数操作, 140 条流水和 9 条访问码记录的完整摘要不变. 核对本机与正式公网首页, 健康检查, 就绪检查及构建文件均正常, 保持现有隧道容器.

按 `/tmp/sprout-series-regression/`, `/tmp/sprout-series-final/`, `/tmp/sprout-series-linux/`, `/tmp/sprout-series-docker/` 和 `/tmp/sprout-series-deployment.json` 复核结果. 保留备份 `$HOME/.local/share/sprout-canvas/backups/series-review-2026-09-16T07-41-07-959Z/`, 按用户授权再提交或推送.

### 8.14 自由系列策划与多图参考验收, 2026-09-16

- [x] 移除四类应用场景预设, 按输入用途和完整程度策划. 验证长剧本, 标题冒号, 分镜换行, 品牌与占位符原文进入文本请求, 拆解后仍需用户确认生图. 将 16000 字输入与每镜 5000 字上限纳入契约检查, 不把虚拟文本返回视为真实模型遵循质量验证.
- [x] 验证最多 4 张参考图的多选添加, 逐张更换/移除, 清空和刷新恢复; 原生粘贴与拖拽仅在空区接受首图, 已有图片时不覆盖, 普通文字粘贴正常. 超量或任一文件读取失败时保留原列表.
- [x] 验证每镜完整保留 4 张用户图片, 将首镜或重绘原图独立追加在末尾. 核对 Images multipart 与 Responses 图片顺序, 仍按输出张数扣点; 首镜已领取后的恢复和换码确认重建不丢图, 不重复扣点.
- [x] 验证衔接图保存到本地配方且不被素材清理误删. 复用完整系列配方到单图时展示全部 5 张素材, 超量阻止生成, 移除多余图片后恢复提交, 不自动发起生成.
- [x] 验证旧单图系列草稿可读取, 更新拆解指令后待领取文字复用原请求, 本地保存失败与迟到结果不重复调用或覆盖新稿. 保留原首镜失败重试, 单镜版本, 排队编辑与结果领取回归.
- [x] 通过 115 项 Node 测试与 TypeScript/Vite 构建, 本机 Chrome 的 124 组浏览器检查, Linux Chromium 的 39 组直接相关回归, Node 22.23.2 独立 Docker 的 13 项验收. 全部生成场景使用隔离数据与虚拟上游, 未调用真实付费模型.
- [x] 在无活动任务及待领取图片时备份并更新本机应用容器. 核对 84 条任务, 59 条点数操作, 140 条流水, 9 条访问码和 60 款风格摘要不变, 配置与隧道容器保持不变. 验证本机和正式公网首页, 健康检查, 就绪检查及构建文件均正常.

按 `/tmp/sprout-free-series-unit-final.log`, `/tmp/sprout-free-series/final/`, `/tmp/sprout-free-series/linux/`, `/tmp/sprout-free-series/docker/` 和 `/tmp/sprout-free-series/deployment/` 复核结果. 保留备份 `$HOME/.local/share/sprout-canvas/backups/free-series-20260916-r8vUeR/`, 按用户授权再提交或推送.

### 8.15 公共参考图槽位与大图预览验收, 2026-09-16

- [x] 在单图创作与系列策划共用四个横向参考图槽位. 验证空态及载入 2/4 张图片后保持同排, 等宽和固定高度, 320px/390px 窄屏四槽同时可见且页面无横向溢出.
- [x] 验证空位添加, 逐张更换/移除, 多选上传, 空区粘贴/拖拽与草稿恢复. 保留单图多张参考中选择一张局部重绘的流程; 复用系列完整五份素材时保留第五张并沿原规则阻止超量提交.
- [x] 验证大图预览不受配置区边界限制, 支持缩放, 滚轮平移, 切图和适应窗口. 核对图片缩放不改变工具按钮尺寸, 切图重置查看状态, Esc 关闭后恢复焦点, 预览期间快捷键不触发生图且粘贴/拖拽不修改参考图.
- [x] 通过 115 项 Node 测试与 TypeScript/Vite 构建, 本机 Chrome 的 128 组浏览器检查, 同版本 Linux Chromium 的 36 组直接相关回归, Node 22.23.2 独立 Docker 的 13 项验收. 使用隔离数据与虚拟上游, 不调用真实付费模型.
- [x] 在无活动任务和待领取图片时完成 SQLite 在线备份并更新本机应用容器. 核对 84 条任务, 59 条点数操作, 140 条流水, 9 条访问码和 60 款风格摘要不变, 核对配置和隧道容器不变. 验证本机与正式公网首页, 健康检查, 就绪检查及前端构建文件一致.

按 `/tmp/sprout-reference-slots-unit.log`, `/tmp/sprout-reference-slots/final/`, `/tmp/sprout-reference-slots/linux/`, `/tmp/sprout-reference-slots/docker/` 和 `/tmp/sprout-reference-slots/deployment/` 复核结果. 保留备份 `$HOME/.local/share/sprout-canvas/backups/reference-slots-2026-09-16-4BeBfI/`, 按用户授权再提交或推送.

### 8.16 参考图追加与局部重绘交互验收, 2026-09-16

- [x] 验证单图与系列共用粘贴/拖拽追加行为, 无需先聚焦提示词, 已有图片保持顺序和内容. 对满额和超量批次拒绝整批并提示容量, 保留普通文字粘贴与读取失败保护, 刷新后完整恢复.
- [x] 验证一张和多张参考图切换局部重绘均不自动弹窗, 通过槽位下的选择按钮进入编辑. 在保存成功前保持原图选择, 生成参数和草稿不变, 取消与 Esc 均不提交修改或触发生图.
- [x] 验证关闭或保存蒙版后保持四槽位与全部图片, 高亮当前编辑原图. 切换原图以空蒙版开始, 取消后保留原有蒙版, 保存后正确切换原图且刷新可恢复. 在编辑模式禁止粘贴/拖拽追加, 在桌面及 320px/390px 窄屏检查布局.
- [x] 通过 115 项 Node 测试与 TypeScript/Vite 构建, 本机 Chrome 的 131 组浏览器检查, 同版本 Linux Chromium 的 39 组相关回归, Node 22.23.2 独立 Docker 的 13 项验收. 核对局部重绘仅提交选中原图, 保留原作品参数与既有计费规则; 全部自动生成场景使用虚拟上游.
- [x] 在无活动任务及待领取图片时完成备份并更新本机应用容器. 核对 84 条任务, 59 条点数操作, 140 条流水, 9 条访问码和 60 款风格摘要不变, 核对配置与隧道容器不变. 验证本机和正式公网首页, 健康检查, 就绪检查及前端构建资源一致.

按 `/tmp/sprout-reference-interaction-unit.log`, `/tmp/sprout-reference-interaction/final/`, `/tmp/sprout-reference-interaction/linux/`, `/tmp/sprout-reference-interaction/docker/` 和 `/tmp/sprout-reference-interaction/deployment/` 复核结果. 保留备份 `$HOME/.local/share/sprout-canvas/backups/reference-interaction-2026-09-16-Q6yf0B/`, 按用户授权再提交或推送.

### 8.17 局部重绘单张原图布局验收, 2026-09-16

- [x] 将局部重绘恢复为单张原图卡片, 从参考模式切入时固定使用第一张图片, 沿用其参数并清除旧蒙版. 移除多图槽位和原图选择入口, 点击绘制/编辑蒙版才打开工作区, 原图预览仅显示当前图片.
- [x] 验证取消编辑保留进入弹窗前的草稿, 保存与刷新保留原图和蒙版, 重复点击已选中的局部重绘标签不清空蒙版. 验证切回参考模式恢复全部图片顺序, 移除首图后再次进入使用新的首图且不串用旧蒙版.
- [x] 验证重绘请求只包含当前原图和蒙版, 其余素材保留在草稿中. 保留单图与系列的四槽布局, 粘贴/拖拽追加和超量拒绝; 检查桌面与 320px/390px 窄屏卡片, 操作按钮和单图预览.
- [x] 通过 115 项 Node 测试与 TypeScript/Vite 构建, 本机 Chrome 的 131 组浏览器检查, 同版本 Linux Chromium 的 39 组相关回归, Node 22.23.2 独立 Docker 的 13 项验收. 在文字恢复测试中同时等待浏览器请求记录和上游执行, 再读取请求编号; 验证恢复与计费场景通过. 使用隔离数据和虚拟上游.
- [x] 在无活动任务及待领取图片时备份并更新本机应用容器. 核对 84 条任务, 59 条点数操作, 140 条流水, 9 条访问码和 60 款风格摘要不变, 核对配置与隧道容器不变. 验证本机和正式公网首页, 健康检查, 就绪检查及前端构建资源一致.

按 `/tmp/sprout-first-image-edit-unit.log`, `/tmp/sprout-first-image-edit/final/`, `/tmp/sprout-first-image-edit/linux/`, `/tmp/sprout-first-image-edit/docker/` 和 `/tmp/sprout-first-image-edit/deployment/` 复核结果. 保留备份 `$HOME/.local/share/sprout-canvas/backups/first-image-edit-2026-09-16-fy7Rr9/`, 按用户授权再提交或推送.
...
| 部署与持续验证 | Node 115 项, 本轮 Chrome 131 组及 Linux 39 组, Docker 13 项与本机升级验收通过; 正式公网首页和健康检查返回 200, 构建资源与本机一致 | 推送后核对对应提交的远端 Verify; 迁至其他部署环境时单独验收 |
