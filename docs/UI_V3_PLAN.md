...
- [x] 7.1 新建 `src/pages/StylesLibrary.tsx` + 路由接入; 吸顶搜索条 (⌘K 聚焦) + 分类 pills 横滑 (数量徽标) + 视图切换
- [x] 7.2 风格卡网格 (3/2/1 列): 普通卡 (名称/英文名/描述/标签) + 高级卡 (示例图 + 用法说明); 选中态
- [x] 7.3 卡片动作: 复制提示词 (写剪贴板 + toast) / 发送到创作台 (跳页 + 选中持久化)
- [x] 7.4 详情灯箱: 大图 + 完整模板 (mono 块 + 复制) + 用法说明
- [x] 7.5 示例图自托管: 高级风格示例图存 `public/styles/` (Stitch 外链图不可用); 风格数据源复用 `src/lib/styles/image-styles.ts`
- [x] 7.6 验收: 搜索/分类/复制/发送全通; 创作页能看到已选风格 pill
- [x] 7.7 提交: 主题「feat(styles): styles library page」

## 阶段 8 · 展馆页 + 大图查看器

- [x] 8.1 GalleryGrid 重写: 按天分组流 (今日/近 7 天/更早) + sticky 组头 + 计数徽标; 卡片 (图 + prompt 摘要 + 元数据胶囊, 无服务商信息)
- [x] 8.2 大图查看器升级: 左右切换 (按钮 + 键盘 ←→) / 缩放 / prompt 全文 / 下载 (文件名含 prompt 摘要与日期) / 删除 / ESC
- [x] 8.3 单张删除 + 全部清空 (确认对话框, focus trap)
- [x] 8.4 验收: 生成→入馆→查看→下载→删除全链路
- [x] 8.5 提交: 主题「feat(gallery): gallery page + viewer v3」

## 阶段 9 · 切图工具抽屉 + 收尾

- [x] 9.1 SplitTool 改造: 独立页 → 创作页工具抽屉/弹层 (入口: 垫图区 + 展馆卡动作)
- [x] 9.2 功能迁移: 行×列切分 + 切片网格 + 下载 + 「作为参考图送创作」
- [x] 9.3 清理: 删除残留旧 CSS 类 / 无用组件 / 死代码; `tsc -b` 零警告
- [x] 9.4 全量验收: `npm test` 全绿 + 四页 + 双主题 + 响应式断点手动过一遍
- [x] 9.5 文档收尾: PRD/DESIGN 变更记录登记实现完成; AGENTS.md 页面结构描述同步最终态
- [x] 9.6 提交: 主题「feat: UI v3 complete」
...
- 2026-09-06: 全部 9 阶段完成. 65/65 项勾选. 过渡层 legacy-compat.css 全量删除; 旧资产 (js/app.js, css/styles.css, RegionEditor, QueueDock, StylePicker, SplitTool 页面, Card) 全部清理; 最终 CSS 52.6KB (gzip 8.8KB).
