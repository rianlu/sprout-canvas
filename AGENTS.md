...
- 让润色按原文完整程度调整, 保留语言, 主体, 数量, 品牌, 引用文字, 占位符与限制, 不强行压缩到一句话或 60 字. 校验非空与生成长度上限后才结算, 不截断结果后收费. 将结果和最近一次前后版本保存到当前草稿后再结束领取, 保存失败或指令更新时复用原请求; 迟到结果不得覆盖已切换或其他标签页更新的草稿. 允许免费撤销/恢复并保留两侧手工修改, 不因撤销退还成功调用点数.
