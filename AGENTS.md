...
| `src/app/App.tsx` | 四页导航, 登录, 队列和作品动作集成 |
...
7. 将原图/参考图/蒙版保存在浏览器. 服务端图片只在内存短期中转, 通常 30 分钟, 领取且无活动依赖后释放.
...
- 从 `src/lib/styles/catalog.json` 读取 36 款风格和真实分类数量, 保留素材署名和 CC BY 4.0 许可.
