...
- 将单图画卷限制为最多 4 个创作记录, 优先当前批次并以近期作品补位, 同一任务的排队/生成/保存/结果原位更新. 保留完整本地展馆和队列记录; 两栏随整页滚动, 不使用控制栏吸顶或双区独立滚动.
...
- 仅在单图参考区完全为空且未编辑蒙版时接收图片粘贴/拖拽. 已有图片时忽略且不弹窗, 连续输入只载入首张; 保留普通文字粘贴与手动更换, 不隐式覆盖参考图或蒙版. 不恢复基础画风快捷项, 风格模板继续来自风格库.
- 通过明确的多选/添加入口支持最多 4 张参考图, 每张不超过 12 MiB, 按顺序逐张管理并完整保存到草稿/outbox/配方. 从多图进入局部重绘时选择一张原图, 未选定前阻止生成; 仅提交所选原图和蒙版, 其余参考留在草稿中供切回使用. 保持现有单份创作草稿和模式切换, 不引入双模式独立草稿. 参考模式通过顶部局部重绘切换, 不重复展示涂抹按钮; 仅在编辑模式展示绘制蒙版或编辑蒙版操作. 按输出张数计费.
