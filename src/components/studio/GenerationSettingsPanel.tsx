  ...
  return (
    <div className="generation-toolbar">
      <div className="field-grid settings-grid">
        ...
        <label className="field"><span>张数</span><input type="number" min={1} max={8} value={config.imageCount} onChange={(event) => onChange({ imageCount: Number(event.target.value) || 1 })} /></label>
      ...
      <details className="advanced-settings inline-advanced">
        ...
        <div className="field-grid">
          ...
          <label className="field"><span>压缩率</span><input type="number" min={10} max={100} value={config.outputCompression} onChange={(event) => onChange({ outputCompression: Number(event.target.value) || 90 })} /></label>
        </div>
      </details>
