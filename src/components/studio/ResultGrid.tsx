...
export function ResultGrid({ records, jobs = [], onDelete, onRetry }: ResultGridProps) {
    ...
    <section className="result-grid current-task-grid">
      ...
      {visibleJobs.map((job) => (
        <article className={`result-card task-card ${job.status}`} key={job.id}>
          ...
          {job.status === 'failed' && onRetry && <div className="result-actions"><Button variant="ghost" onClick={() => onRetry(job.id)}>换服务商重试</Button></div>}
        </article>
      ))}
      {records.map((record) => (
