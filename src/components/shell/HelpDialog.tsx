  ...
  return <div className="fixed inset-0 z-50 bg-inverse-surface/40 backdrop-blur-md flex items-center justify-center p-4" onClick={onClose}>
    <div ref={ref} role="dialog" aria-modal="true" aria-label="工作台帮助" className="w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl bg-surface p-space-lg shadow-xl" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="space-y-space-md text-body-sm leading-relaxed">
        ...
        <section><h3 className="font-medium mb-1">作品保存在此浏览器</h3><p>创作画卷最多显示 4 个近期记录, 当前批次优先. 全部已保存作品可前往展馆查看和下载. 清理浏览器数据会清除作品, 喜欢的图片请及时下载.</p></section>
        <section><h3 className="font-medium mb-1">排队与恢复</h3><p>顶部任务队列可查看其他任务和失败原因. 刷新或关闭页面不会取消已经提交的任务, 也不会重复扣点, 请及时回到此浏览器领取结果. 排队任务可以取消或在自己的队列中置顶, 已开始的生成无法撤回. 服务重启后的未执行任务可恢复, 结果未知的任务会单独提示.</p></section>
        ...
        <section><h3 className="font-medium mb-1">文件与分享</h3><p>PNG 和 WebP 可用于透明背景, JPEG 不保留透明通道. 查看器显示实际图片尺寸与生成配方. 作品工具菜单中的切图拆分可按行列切分图片, 一次打包下载全部切片为 ZIP, 也可逐张下载或送回创作台. 切图在浏览器完成, 不消耗灵感点. 支持文件分享的浏览器会显示系统分享入口, 图片不会上传到工作台服务器生成链接.</p></section>
