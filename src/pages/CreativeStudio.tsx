...
export function CreativeStudio({ onSubmit, onRetry, results, jobs }: CreativeStudioProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setToast({ type: 'success', message: `已提交 ${snapshot.imageCount} 个任务, 可以继续创作.` });
    } catch (error) {
  ...
  const showEditEditor = config.mode === 'edit' && submittedIds.length === 0;
  return (
    <div className="studio-page">
      ...
      <div className="studio-workbench">
        ...
        <section className="studio-preview">
          ...
          {showEditEditor ? <RegionEditor image={editImage} selection={config.editSelection || null} onSelectionChange={(editSelection) => patch({ editSelection })} /> : <ResultGrid records={currentResults} jobs={currentJobs} onRetry={(jobId) => { void onRetry(jobId).then((job) => {
