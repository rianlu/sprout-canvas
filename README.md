...
在同一页完成提示词润色, 参考图输入和生成配置. 创作画卷展示最近 4 个记录, 完整作品保存在展馆.

[![单图创作: 提示词润色与撤销, 参考图输入和近期创作画卷](docs/images/studio.webp)](docs/images/studio.webp)
...
- **风格库**: 首次启动导入 36 款 / 6 类开源案例, 后续由后台维护, 更新与重启不会覆盖编辑结果. 支持搜索, 分类和原文复制, 可直接发送到单图创作.
